...
const SYNC_PREF_KEYS = {
  ...
  hero_period:'prizm_hero_period', merchant_cat:'prizm_merchant_cat'
};
...
function renderHome() {
  ...
  (function () {
    const gs = document.getElementById('home-getstarted');
    const show = !S.txs.length;
    if (gs) gs.style.display = show ? '' : 'none';
    const rsh = document.getElementById('recent-sh'); if (rsh) rsh.style.display = show ? 'none' : '';
    const txEl = document.getElementById('home-txs'); if (txEl && show) txEl.style.display = 'none'; else if (txEl) txEl.style.display = '';
    if (show) {
      const done = 1 + ((S.accts && S.accts.length) ? 1 : 0); // welcome + (account?)
      const pct = Math.round(done / 3 * 100);
      const bar = document.getElementById('gs-bar'); if (bar) bar.style.width = pct + '%';
      const p = document.getElementById('gs-pct'); if (p) p.textContent = pct + '%';
      const acctStep = document.getElementById('gs-acct');
      if (acctStep) {
        const o = acctStep.querySelector('.gs-o');
        if (o) {
          if (S.accts && S.accts.length) { o.style.background = 'var(--green)'; o.style.border = 'none'; }
          else { o.style.background = 'transparent'; o.style.border = '2px solid var(--line-3)'; }
        }
      }
    }
  })();
...
function _loadMerchMem() { try { return JSON.parse(localStorage.getItem('prizm_merchant_cat') || '{}'); } catch (e) { return {}; } }
function learnMerchant(desc, category) {
  const k = _merchKey(desc);
  if (!k || k.length < 3 || !category) return;
  const m = _loadMerchMem();
  if (m[k] === category) return;
  m[k] = category;
  try { localStorage.setItem('prizm_merchant_cat', JSON.stringify(m)); } catch (e) {}
...
function guessCategory(desc) {
  ...
  if (mem[k]) return mem[k];                                   // exact learned merchant
  for (const key in mem) { if (key.length >= 4 && (k.indexOf(key) !== -1 || key.indexOf(k) !== -1)) return mem[key]; } // fuzzy learned
...
async function submitTx() {
  ...
  if (desc && S.cat) learnMerchant(desc, S.cat); // remember this merchant → category
  if (editingId) {
